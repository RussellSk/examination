...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Правильные результаты</h1>
    </div>

    <a href="{{ route('correct.import') }}" class="btn btn-success mb-3">Импортировать студентов</a>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover table-sm">
                            ...
                            @foreach ($results as $result)
                                <tr>
                                    <td>{{ $result->id }}</td>
                                    <td>{{ $result->student->name ?? '' }} ({{ $result->student_code }})</td>
                                    <td>{{ $result->total_answers }}</td>
                                    <td>{{ $result->given_answers }}</td>
